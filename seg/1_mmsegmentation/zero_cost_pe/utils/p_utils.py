...
def get_some_data(train_dataloader, num_batches, device):
    ...
    inputs = torch.cat([a['img'].data[0] for a in traindata])
    img_metas = ([a['img_metas'].data[0] for a in traindata])
    targets = torch.cat([b['gt_semantic_seg'].data[0] for b in traindata])
    inputs = inputs.to(device)
    targets = targets.to(device)
    return inputs,img_metas, targets
