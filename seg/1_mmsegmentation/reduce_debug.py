...
def main():
    ...
    # checkpoint = load_checkpoint(model, cfg.unfixed_ckpt, map_location='cpu')
    ...
    for i in range(50):
        while True:
            ...
            if True or 0.9 * cfg.constraint_flops < flops < cfg.constraint_flops:
                break
            else:
        ...
        results = get_zc_pe(subnet.module, subnet.module.decode_head.losses,train_loader, model.CLASSES,  cfg.device)
        ...
        if args.eval:
            ...
            metric = results # val_dataset.evaluate(results, **eval_kwargs)
            mIoU = metric               
            benchmarks.append({
            ...
            logger.info('Initail population: {}th subnet, score: {} flops: {} subnet_cfg:{}'.format(i, mIoU, flops, subnet_cfg))      
    ...
    features = []
    # subnets = []
    total_num = int(20*256/(1-ratio))# search space size (about 20*128 subnet to eval)
    total_num = 50
    ...
    for idx in idxs:
        ...
        if (results > thres).sum(axis=1) < 1.5:
            continue
        # scores.append(results)
        ...
        logger.info('second PE: {}th subnet, score: {} flops: {} subnet_cfg:{}'.format(i, mIoU, flops, subnet_cfg))   
        suggest_samples.append(benchmarks[-1])
    with open('./res.pkl', 'wb') as f:
        pickle.dump(suggest_samples, f)
    with open('./res.pkl', 'rb') as f:
        suggest_samples = pickle.load(f)
    ...
    with torch.no_grad():
        ...
        for batch_idx, x in enumerate(train_loader):
            ...
            if batch_idx >= post_bn_calibration_batch_num:
                break
            model.module.forward_before_global_avg_pool(img=img, img_metas=img_metas, return_loss=False)  #forward only     
