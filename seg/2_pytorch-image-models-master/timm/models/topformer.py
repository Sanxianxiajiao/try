...
default_cfgs = {
    'topformer_base': _cfg(),
    'topformer_small': _cfg(),
    'topformer_tiny': _cfg(),
    'topformer_search': _cfg(),
    'topformer_search_5k': _cfg(),
    'topformer_search_5k_evo': _cfg(),
    'topformer_search_small': _cfg(),
    'topformer_search_tiny': _cfg()
}
...
def _create_topformer(variant, pretrained=False, **kwargs): # variant (str): model variant name
    return build_model_with_cfg(Topformer, variant, pretrained, **kwargs)

@register_model
def topformer_base(pretrained=False, **kwargs):
    """Constructs a topformer-base model.
    """
    model_cfgs = dict(
        stem_channel=16,
        cfg=[
        ...
        embed_out_indice=[1, 3, 5, 9], # TODO bug fixed
        decode_out_indices=[1, 2, 3],
        key_dim=[18, 16, 16, 16],
        num_heads=[10, 8, 12, 4],
    ...
    return _create_topformer('topformer_base', pretrained, **model_args)

@register_model
def topformer_small(pretrained=False, **kwargs):
    """Constructs a topformer-small model.
    """
    model_cfgs = dict(
        stem_channel=16,
        cfg=[
    ...
    return _create_topformer('topformer_small', pretrained, **model_args)

@register_model
def topformer_tiny(pretrained=False, **kwargs):
    """Constructs a topformer-tiny model.
    """
    model_cfgs = dict(
        stem_channel=16,
        cfg=[
        # k,  t,  c, s
            [3,   1,  16, 1], # 1/2        0.464K  17.461M
            [3,   3,  24, 2], # 1/4 1      3.44K   64.878M
            # [3,   3,  16, 1], #            4.44K   41.772M
            [5,   2,  32, 2], # 1/8 3      6.776K  29.146M
            # [5,   3,  32, 1], #            13.16K  30.952M
            [3,   2,  56, 2], # 1/16 5     16.12K  18.369M
            [3,   2,  56, 1], #            41.68K  24.508M
            [3,   2,  56, 1], #            41.68K  24.508M
            [5,   6,  104, 2], # 1/32 7     0.129M  36.385M
            [5,   6,  104, 1], #            0.335M  49.298M
            [5,   6,  104, 1], #            0.335M  49.298M
        ],
    ...
    return _create_topformer('topformer_tiny', pretrained, **model_args)

@register_model
def topformer_search(pretrained=False, **kwargs):
    """Constructs a topformer-search model.
    """
    model_cfgs = dict(
        stem_channel=16,
        cfg=[
    ...
    return _create_topformer('topformer_search', pretrained, **model_args)

@register_model
def topformer_search_5k(pretrained=False, **kwargs):
    """Constructs a topformer-search-5k model.
    """
    model_cfgs = dict(
        stem_channel=24,
        cfg=[
    ...
    return _create_topformer('topformer_search_5k', pretrained, **model_args)

@register_model
def topformer_search_5k_evo(pretrained=False, **kwargs):
    """Constructs a topformer-search-5k model.
    """
    model_cfgs = dict(
        stem_channel=24,
        ...
        channels=[32, 64, 128, 176],
        out_channels=[None, 256, 256, 256],
        embed_out_indice=[2, 4, 5, 9],
        decode_out_indices=[1, 2, 3],
        key_dim=[16, 18, 16, 14], 
        num_heads=[4, 4, 6, 8],
        attn_ratios=[1.8, 1.8, 2.4, 2.4],
        mlp_ratios=[2.2, 1.8, 1.6, 2.0],
        c2t_stride=1,
    )
    model_args = dict(
        cfgs=model_cfgs['cfg'], 
...
@register_model
def topformer_search_small(pretrained=False, **kwargs):
    """Constructs a topformer-search-small model.
    """
    model_cfgs = dict(
        stem_channel=16,
        cfg=[
    ...
    return _create_topformer('topformer_search_small', pretrained, **model_args)

@register_model
def topformer_search_tiny(pretrained=False, **kwargs):
    """Constructs a topformer-search-tiny model.
    """
    model_cfgs = dict(
        stem_channel=16,
        cfg=[
    ...
    return _create_topformer('topformer_search_tiny', pretrained, **model_args)