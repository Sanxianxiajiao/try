...
default_cfgs = {
    'nas_dynamic_topformer_fix_backbone': _cfg(),
    'nas_dynamic_topformer_fix_trans': _cfg(),
    'nas_dynamic_topformer': _cfg(),
    'nas_dynamic_topformer_small': _cfg(),
    'nas_dynamic_topformer_tiny': _cfg()
}
 
class NASDynamicTopFormer(BaseModule):
    def __init__(self,
                 supernet=None,
                 fix_backbone=False,
                 fix_trans=False,
    ...
    def set_active_subnet(self, width=None, depth=None, kernel_size=None, expand_ratio=None, \
        ...
        assert len(depth) == len(kernel_size) == len(expand_ratio) == len(width) - 1 # attentivenas中是-2，由于topformer中没有last_conv 所以是-1
        '''tpm'''
        # set channel split
        self.dynamic_channel_split = []
        # first conv
    ...
    def get_active_subnet(self, preserve_weight=True): 
        # 调用各个模块的get_active_subnet方法, 获取出的子网要保留权重，
        # eval时在model_factory.py中被调用
        # 写死的模块直接传入StaticTopFormer
        with torch.no_grad():
            '''tpm'''
            ...
            _subnet = NASStaticTopFormer(stem=stem,  
...
def _create_nas_dynamic_topformer(variant, pretrained=False, **kwargs): # variant (str): model variant name, topformer_base
    return build_model_with_cfg(NASDynamicTopFormer, variant, pretrained, **kwargs)

# fix backbone model
@register_model
def nas_dynamic_topformer_fix_backbone(pretrained=False, **kwargs):
    """Constructs a nas_dynamic_topformer_fix_backbone model.
    """
    supernet=dict(
    ...
    return _create_nas_dynamic_topformer('nas_dynamic_topformer_fix_backbone', pretrained, **model_args)

# fix trans model
@register_model
def nas_dynamic_topformer_fix_trans(pretrained=False, **kwargs):
    """Constructs a nas_dynamic_topformer_fix_trans model.
    """
    supernet=dict(
        stem=dict(c=[16, 24], act_func='relu6', s=2),
        embed_out_indice=[1, 2, 3, 4],
    ...
    return _create_nas_dynamic_topformer('nas_dynamic_topformer_fix_trans', pretrained, **model_args)


@register_model
def nas_dynamic_topformer(pretrained=False, **kwargs):
    """Constructs a nas_dynamic_topformer model.
    """
    supernet=dict(
        stem=dict(c=[16, 24], act_func='relu6', s=2),
        embed_out_indice=[1, 2, 3, 4],
    ...
    return _create_nas_dynamic_topformer('nas_dynamic_topformer', pretrained, **model_args)

@register_model
def nas_dynamic_topformer_small(pretrained=False, **kwargs):
    """Constructs a nas_dynamic_topformer_small model.
    """
    supernet=dict(
    ...
    return _create_nas_dynamic_topformer('nas_dynamic_topformer_small', pretrained, **model_args)

@register_model
def nas_dynamic_topformer_tiny(pretrained=False, **kwargs):
    """Constructs a nas_dynamic_topformer_tiny model.
    """
    supernet=dict(
        stem=dict(c=[16], act_func='relu6', s=2),
        embed_out_indice=[1, 2, 3, 4],
    ...
    return _create_nas_dynamic_topformer('nas_dynamic_topformer_tiny', pretrained, **model_args)
