...
class TrainMeter(object):
    ...
    def get_iter_stats(self, cur_epoch, cur_iter, win_avg=True):
        ...
        stats = {
            ...
            "top1_err": self.mb_top1_err.get_win_avg() if win_avg else self.mb_top1_err.get_win_last(),
            "top5_err": self.mb_top5_err.get_win_avg() if win_avg else self.mb_top5_err.get_win_last(),
            "loss": self.loss.get_win_avg() if win_avg else self.loss.get_win_last(),
            "lr": self.lr,
            "mem": int(np.ceil(mem_usage)),
        }
        return stats

    def log_iter_stats(self, cur_epoch, cur_iter, stats=None):
    ...
    def log_epoch_stats(self, cur_epoch, stats=None):
        if stats is None:
