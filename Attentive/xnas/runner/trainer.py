...
class Recorder():
    ...
    def __init__(self):
        ...
        self.writer = MySummaryWriter(SummaryWriter(log_dir=os.path.join(cfg.OUT_DIR, "tb")))
...
class Trainer(Recorder):
    ...
    def test_epoch(self, cur_epoch, rank=0):
        ...
        self.writer.add_scalar('val/top1_error', top1_err, cur_epoch)
        self.writer.add_scalar('val/top5_error', top5_err, cur_epoch)
        # Log epoch stats
    ...
    def saving(self, epoch, best=False, ckpt_dir=None):
        """Save to checkpoint."""
        if not multi_process.allow_save_checkpoint():
            return
        if not ((epoch + 1) % cfg.SAVE_PERIOD == 0 or (epoch + 1) == cfg.OPTIM.MAX_EPOCH):
            return 
        _kwdict = {}
