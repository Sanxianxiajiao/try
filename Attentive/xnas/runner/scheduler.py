...
class GradualWarmupScheduler(_LRScheduler):
    ...
    Args:
        ...
        warmup_lr: start the warm up from warmup_lr to base_lr
        actual_scheduler: after warmup_epochs, use this scheduler
        warmup_epochs: base_lr is reached at warmup_epochs, linearly
    """
    ...
    def get_lr(self):
        if self.last_epoch > self.warmup_epochs:
            ...
            return [((base_lr - self.warmup_lr) * self.last_epoch / self.warmup_epochs + self.warmup_lr) for base_lr in self.base_lrs]
...
def _warmup_adjust_learning_rate(
        base_lr, n_epochs, epoch, n_iter, iter=0, warmup_lr=0
    ):
        """adjust lr during warming-up. Changes linearly from `warmup_lr` to `base_lr`."""
        T_cur = epoch * n_iter + iter + 1
        t_total = n_epochs * n_iter
        new_lr = T_cur / t_total * (base_lr - warmup_lr) + warmup_lr
        return new_lr
...
def adjust_learning_rate_per_batch(epoch, n_iter=None, iter=0, warmup=False):
    ...
    warmup_lr = cfg.OPTIM.WARMUP_LR
    
    if warmup:
        new_lr = _warmup_adjust_learning_rate(
            base_lr, n_warmup_epochs, epoch, n_iter, iter, warmup_lr
        )
    else:
        new_lr = _calc_learning_rate(
            base_lr, n_epochs, epoch, n_iter, iter
        )
    return new_lr